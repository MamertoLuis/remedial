...
from .models import LoanAccount, CollectionActivityLog, Borrower, Exposure, DelinquencyStatus, RemedialStrategy
...
class BorrowerTable(tables.Table):
    ...
    class Meta:
        model = Borrower
        fields = ('borrower_id', 'full_name', 'borrower_type', 'mobile', 'email')
...
class CollectionActivityLogTable(tables.Table):
    ...
    class Meta:
        ...
        fields = ('activity_date', 'activity_type', 'staff_assigned', 'remarks', 'promise_to_pay_date', 'next_action_date', 'actions')
        attrs = {'class': 'table table-striped'}
...
class BorrowerAccountTable(tables.Table):
    ...
    class Meta:
        model = LoanAccount
        fields = ('loan_id', 'loan_type', 'booking_date', 'status', 'original_principal', 'interest_rate')
        attrs = {'class': 'table table-striped'}
