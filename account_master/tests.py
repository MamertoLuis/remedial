...
from account_master.models import (
    Borrower, LoanAccount, Exposure, DelinquencyStatus, 
    RemedialStrategy, CollectionActivityLog
)
...
class AccountDetailViewTests(TestCase):
    ...
    def test_account_detail_view_authenticated(self):
        ...
        response = self.client.get(reverse('account_detail', kwargs={'loan_id': self.loan_account.loan_id}))
    ...
    def test_account_detail_view_context(self):
        ...
        self.assertEqual(response.context['latest_delinquency'], self.latest_delinquency)
        ...
        self.assertEqual(len(response.context['historical_strategies_table'].data), 1) # Only historical, not active
    ...
    def test_account_detail_view_content(self):
        ...
        self.assertContains(response, self.loan_account.pn_number)
        self.assertContains(response, "₱100,000.00") # Using intcomma filter
        self.assertContains(response, self.loan_account.get_status_display()) # "Performing"
        ...
        self.assertContains(response, f"{self.latest_delinquency.days_past_due} Days Past Due")
        self.assertContains(response, self.latest_delinquency.get_aging_bucket_display()) # "30 days"
        self.assertContains(response, self.latest_delinquency.get_classification_display()) # "Especially Mentioned"

        # Check Current Remedial Strategy Card
        self.assertContains(response, "Current Strategy")
        self.assertContains(response, "Start Date:")
        self.assertContains(response, self.current_strategy.strategy_start_date.strftime('%b'))
        self.assertContains(response, str(self.current_strategy.strategy_start_date.day))
        self.assertContains(response, str(self.current_strategy.strategy_start_date.year))
        self.assertContains(response, self.current_strategy.get_strategy_type_display()) # "Restructuring"
        self.assertContains(response, self.current_strategy.get_strategy_status_display()) # "Active"
        ...
        self.assertContains(response, "Intensive Collection") # Historical strategy in table
        self.assertContains(response, "Cancelled") # Historical strategy status in table
        self.assertContains(response, "Called borrower, no answer") # From collection activities
        self.assertContains(response, "Visited borrower, discussed payment") # From collection activities