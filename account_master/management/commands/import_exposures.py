...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'Attempting to import exposure data from {csv_file_path}...'))
                    ...
                    try:
                        with transaction.atomic():
                            ...
                            try:
                                ...
                                raise ValueError(f"Invalid date format for 'as_of_date': {as_of_date_str}. Expected YYYY-MM-DD.")

                            # Parse decimal fields
                            principal_outstanding = Decimal(row.get('principal_outstanding', '0.00'))
                            accrued_interest = Decimal(row.get('accrued_interest', '0.00'))
                            accrued_penalty = Decimal(row.get('accrued_penalty', '0.00'))
                            legal_fees = Decimal(row.get('legal_fees', '0.00'))
                            other_charges = Decimal(row.get('other_charges', '0.00'))
                            provision_level = Decimal(row.get('provision_level', '0.00'))
                        ...
                        errors.append(f'Row {row_num}: LoanAccount with loan_id "{loan_id}" does not exist.')
