...
from .models import Borrower, LoanAccount, Exposure, DelinquencyStatus, CollectionActivityLog, RemedialStrategy
...
def upsert_exposure(*, account: LoanAccount, as_of_date: date, defaults: dict) -> tuple[Exposure, bool]:
    """
    ...
    total_exposure = principal_outstanding + accrued_interest + accrued_penalty + legal_fees + other_charges
...
def upsert_delinquency_status(*, account: LoanAccount, as_of_date: date, defaults: dict) -> tuple[DelinquencyStatus, bool]:
    """
...
def create_collection_activity(*, account: LoanAccount, activity_date: date, activity_type: str, remarks: str, **kwargs) -> CollectionActivityLog:
    """
...
def create_remedial_strategy(*, account: LoanAccount, strategy_type: str, strategy_start_date: date, **kwargs) -> RemedialStrategy:
    """
...
def take_snapshot(
    *,
    ...
    upsert_delinquency_status(account=account, as_of_date=as_of_date, defaults=delinquency_data)
